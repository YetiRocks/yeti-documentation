...
For logging, use `tracing::warn!()` or other `tracing` macros. Never use `eprintln!()` -- it bypasses the structured logging pipeline.
...
```
https://localhost/{app-id}/{resource-or-table}
```
